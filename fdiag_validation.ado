capture program drop fdiag_validation
program define fdiag_validation
* version 1.0  AH 04 Oct 2023
...
*qui bysort patient (icd10_date): keep if _n ==1
	...
	* merge events to original dataset 
	restore 
	if "`nogenerate'" =="" { 
		qui merge 1:m patient using "`events'", keep(match master) nogen
		if "`y'" != "" {
